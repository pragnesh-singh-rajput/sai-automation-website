  ...
  return (
    <div className="relative pl-8 sm:pl-10 py-2 group">
      ...
      <div className="flex flex-col items-center w-8 sm:w-10 absolute left-0 top-0 bottom-0 transform -translate-x-1/2">
        <div className="h-full w-px bg-border group-hover:bg-primary smooth-transition"></div>
        <div className="sticky top-1/2 -translate-y-1/2 z-10"> {/* Ensure circle is on top */}
          <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-background border-2 border-primary group-hover:bg-primary smooth-transition flex items-center justify-center">
            <CalendarDays className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-primary group-hover:text-primary-foreground smooth-transition" />
          </div>
        </div>
        {!isLast && <div className="h-full w-px bg-border group-hover:bg-primary smooth-transition"></div>}
      </div>

      {/* Content Card */}
      <Card className="ml-4 sm:ml-6 card-interactive shadow-md hover:shadow-xl">
